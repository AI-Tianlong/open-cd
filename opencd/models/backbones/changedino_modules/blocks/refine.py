...
class LearnableSoftMorph(nn.Module):
    ...
    def _soft_dilate(self, x, k, w, tau):
        ...
        pad = k // 2
        cols = F.unfold(x_reshaped, k, padding=pad)
        cols = cols.view(B * C, 1, k*k, H*W)
    ...
    def forward(self, logits):
        ...
        all_probs = F.softmax(logits, dim=1)  # [B, C, H, W]
        tau = torch.exp(self.log_tau).clamp_min(1e-4)

        # 2. Extract all foreground channels (skip background at index 0)
        # Shape: [B, C-1, H, W]
        fg_probs = all_probs[:, 1:] 
        
        if fg_probs.shape[1] > 0:
            # 3. Apply morphology to all foreground channels in parallel
            # Open: erode -> dilate
            p = self._soft_erode (fg_probs, self.k_open,  self.we_open_erode,   tau)
            ...
            # 4. Convert back to logit space
            fg_logit_refined = torch.logit(p.clamp(1e-6, 1-1e-6), eps=1e-6)

            # 5. Residual connection
            alpha = torch.sigmoid(self.alpha_raw)  # [0,1]
            
            # Clone logits to avoid in-place modification errors if gradients are tracked
            out = logits.clone()
            
            # Apply refinement only to foreground channels
            out[:, 1:] = out[:, 1:] + alpha * (fg_logit_refined - out[:, 1:])
