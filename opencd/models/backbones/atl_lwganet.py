...
class Stem(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.norm(self.proj(x)) # [1,4,512,512]-->[1,96,128,128]-->[1,96,128,128]
        return x
...
class LWGANet(nn.Module):
    ...
    def forward_det(self, x: Tensor) -> Tensor:
        ...
        x = self.Stem(x)  # [1,4,512,512] --> [1,96,128,128]
        outs = []
        for idx, stage in enumerate(self.stages):
            ...
            if self.fork_feat and idx in self.out_indices:
                ...
                x_out = norm_layer(x)
                outs.append(x_out)
        return outs # [1,96,128,128][1,192,64,64][1,384,32,32][1,768,16,16]


def LWGANet_L0_1242_e32_k11_GELU(num_classes=3, stem_dim=32, pretrained=None, **kwargs):
    model = LWGANet(in_chans=4,
...
def LWGANet_L1_1242_e64_k11_GELU(num_classes=3, stem_dim=64, pretrained=None, **kwargs):
    model = LWGANet(in_chans=4,
                    num_classes=num_classes,
                    stem_dim=stem_dim,
                    depths=(1, 2, 4, 2),
...
def LWGANet_L2_1242_e96_k11_RELU(num_classes=3, stem_dim=96, pretrained=None, **kwargs):
    ...
    model = LWGANet(in_chans=4,
                    ...
                    **kwargs)
    if pretrained == True:
        checkpoint = torch.load('/data/AI-Tianlong/2-part2-coastCD/对比实验/LWGANet/change_detection/checkpoints/4chan/lwganet_l2_e296.pth',
                                map_location=torch.device('cuda:0'))
        # _state_dict = checkpoint['model'] # 变成4通道之后，这个没了、
